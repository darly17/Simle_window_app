...
class Controller:
    ...
    def validate_student_data(self, data):
        ...
        if not isinstance(data['exams'], dict):
            ...
            for subject, grade in data['exams'].items():
                if not isinstance(subject, str)  or not all(c.isalpha() or c.isspace() for c in subject) or not subject.strip():
                    errors.append(
    ...
    def search_by_avg_grade(self, ex, min, max):
        try:
            ...
            result = []
            errors = []
            if not isinstance(ex, str)  or not all(c.isalpha() or c.isspace() for c in ex) or not ex.strip():
                    errors.append(
                        "Название предмета должно быть непустой строкой и содержать только буквы и пробелы")
                    
            if not isinstance(min, float) or min < 1 or min > 10:
                errors.append(f"Предел должен быть целым числом от 1 до 10")

            if not isinstance(max, float) or max < 1 or max > 10:
                errors.append(f"Предел должен быть целым числом от 1 до 10")
            if errors:
                return str(errors)
